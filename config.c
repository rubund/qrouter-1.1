/*--------------------------------------------------------------*/
/* config.c -- .cfg file read/write for route                  	*/
/*--------------------------------------------------------------*/
...
{
    int count, lines, i, OK, h, v;
    int iarg, iarg2;
    char carg, gatename[MAX_NAME_LEN];
	...
	if ((i = sscanf(lineptr, "gate %s %lf %lf\n", sarg, &darg, &darg2)) == 3) {
	    ...
	    gateinfo->gatetype = strdup(sarg);
	    gateinfo->gatename = NULL;
	    gateinfo->width = darg;
