...
int eval_pt(int x, int y, int lay, int thisindex, u_char stage)
{
    ...
    if (thiscost < Pr[k].cost) {
       if (Pr[k].pred != thisindex) {
          Pr[k].alt = Pr[k].pred;	// Keep last best route as alternate
          Pr[k].pred = thisindex;
       }
       Pr[k].cost = thiscost;
    ...
    else if ((thiscost == Pr[k].cost) && (thisindex != Pr[k].pred)) {
       Pr[k].alt = thisindex;		// Alternate route has equal cost
    }
...
int commit_proute(ROUTE rt, u_char stage)
{
   ...
   while (1) {
      ...
      if (NoStackedContacts == 1 && lseg && lseg->segtype == ST_VIA
		&& seg->segtype == ST_VIA) {
	 if (Pr[index].alt == 0) {
	    printf("Error:  Unable to avoid stacked via!\n");
	 }
	 else {
	    prev = Pr[index].alt;
            seg->segtype = (ll == Pr[prev].layer) ? ST_WIRE : ST_VIA;
	    if (seg->segtype == ST_VIA) {
	       printf("Error:  Alternate route is also stacked via?!\n");
	    }
	 }
      }
