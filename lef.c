...
{  
	...
	if (errors)
	{
	    printf("LEF Read: encountered %d error%s total.\n", errors,
			(errors == 1) ? "" : "s");
	    errors = 0;
	}
	return;
...
lefFindCell(char *name)
{
	...
	if (!strcasecmp(gateginfo->gatetype, name))
	    return gateginfo;
    }
    return (GATE)NULL;
}
    ...
    float oscale;		/* scale factor to um, usually 1 */
{
    ...
    {
	if (!strcmp(altMacro->gatetype, mname)) {
	    lefMacro = altMacro;
	    break;
	}
    }
...
origin_error:
		...
		*/
		...
		/* Diagnostic */
		/*
		printf("   Macro defines obstruction\n");
		*/
		if (is_imported)
    ...
    char *inName;
{
	...
	switch (keyword)
	{
		...
		token = LefNextToken(f, TRUE);
		printf("LEF file:  Defines site %s (ignored)\n", token);
		sprintf(tsave, "%.127s", token);
		LefSkipSection(f, tsave);
		break;
	    case LEF_PROPERTY:
		...
		/* Diagnostic */
		/*
		printf("LEF file:  Defines new cell %s\n", token);
		*/
		sprintf(tsave, "%.127s", token);
    ...
    printf("LEF read: Processed %d lines.\n", lefCurrentLine);
