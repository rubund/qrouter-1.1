...
void read_def_placement(char *filename)
{
   ...
   while (!feof(pp)) {
	 ...
	 line[j] = fgetc(pp);
	 if (line[j] == '\n') break;
	 if (j != 0 || (line[j] != '\t' && line[j] != ' ')) j++;
      }
      if (comps != 1 && pins != 1) {
	 i = sscanf(line, "COMPONENTS %d ;", &ncells);
         if ((comps == 0) && (i == 1)) comps = 1;
	 else {
	 ...
	 if (!strncmp(line, "END PINS", 8)) {
	    ...
	    i = sscanf(ppos, "LAYER %s", layname);
	    if (i == 0) {
	       if (!strncmp(ppos, "PLACED", 6)) {
		  ppos += 6;
		  i = 1;
	       }
	       else if (!strncmp(ppos, "FIXED", 5)) {
		  ppos += 5;
		  i = 1;
	       }
	       while (*ppos == ' ' || *ppos == '\t') ppos++;
	       if (i == 1)
	          i = sscanf(ppos, " ( %lg %lg ) %s", &x, &y, orient);
		  ...
		  Nlgates = gate;
	       ...
	       i = sscanf(ppos, "LAYER %*s ( %lg %lg ) ( %lg %lg )",
      ...
      else if (pins == 2 && comps == 2) {
         break;  	// Done with all pins and components
      }
   }
   fclose(pp);
